...
router.get('/users/orders', async (req, res) => {
    ...
    const history = await Orders.findAll({where: // busca todos los pedidos que tengan status diferente de cart
        { status: { [Op.ne]: 'Cart' } }, // ne es diferente
...
router.get('/orders', async (req, res, next)=> {
    try{
        const orders = await Orders.findAll({ // busca todas las ordenes
            include: {model: Products} // incluye los productos
        })
        res.send(orders)
    }catch(err){
        next(err);// si hay un error lo envia al middleware
    }
})
